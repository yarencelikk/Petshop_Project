    ...
    if (paymentMethod === "credit_card") {
      ...
      for (let index = 0; index < cart.items.length; index++) {
        ...
        let itemDiscountedPrice = parseFloat(
          (parseFloat(variant.price) * discountRate).toFixed(2),
        );
        for (let i = 0; i < cartItem.quantity; i++) {
          if (index === cart.items.length - 1 && i === cartItem.quantity - 1) {
            const diff = parseFloat(
              (final_price - calculatedPaidPriceTotal).toFixed(2),
            );
          ...
          calculatedPaidPriceTotal = parseFloat(
            (calculatedPaidPriceTotal + itemDiscountedPrice).toFixed(2),
          );
        }
      }
    }
